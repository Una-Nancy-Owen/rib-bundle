import { TimerGroup, Timer, TimerSignal, TimerState } from 'rib-bundle'
...
export const timerController = (nodecg: NodeCG) => {
  ...
  const individualResume = (index: number): void => {
    if (timerState.startedTimer[index]) {
      individualStart(index, true)
    }
  }

  const resume = (): void => {
    timerGroup.timer.forEach((_timer, index) => {
      individualResume(index)
    })
  }
  ...
  const individualReset = (index: number): void => {
    ...
    timerState.individibleTimer[index] = false
    timerState.startedTimer[index] = false
    groupTimerRep.value = timerGroup
    if (timerGroup.runningNum == 0) {
      stopGroupTimer()
    } else {
    ...
    switch (signal.signalType) {
      case 'Start':
        if (signal.all) {
          ...
          individualStart(signal.index, false)
        }
        break
      case 'Stop':
        if (signal.all) {
  ...
  nodecg.listenFor('toggleMSVisibility', (isVisible) => {
