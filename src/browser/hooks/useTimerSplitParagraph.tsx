 ...
 * タイマーを表示するためのコンポーネント
 */
export default function useTimerSplitParagraph(num: number, finishedCol: string = 'rgb(255, 228, 85)', opacue: boolean = false) {
  ...
  const translucent: string = opacue ? '1' : '0.05'
        ...
        if (timerMainRef.current![index] != null && timerSubRef.current![index] != null) {
          if (timerMainRef.current![index].current != null && timerSubRef.current![index].current != null) {
            ...
            timerSubRef.current[index].current!.style.color =
              timer.isRunning || timer.currentTime == 0 ? normalCol : finishedCol
            timerSubRef.current[index].current!.style.opacity = timer.isRunning ? translucent : opacity
          }
        }
      })
    }
