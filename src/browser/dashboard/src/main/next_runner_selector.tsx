...
export const NextRunnerSelector = memo(
  (props: { runnerGroupArray: RunnerGroup[]; currentIndex: number; nextIndex: number }) => {
    ...
    const nextRunnerSelectorContainer = props.runnerGroupArray.map((data) => (
      <RunnerGroupPanel
        key={`nextRunnerSelector${data.group.toString()}`}
        runnerGroup={data}
        currentIndex={props.currentIndex}
        nextIndex={props.nextIndex}
        panelColorStyle={GetPanelColorStyle(props.currentIndex, props.nextIndex, data.group)}
      />
    ))
...
const RunnerGroupPanel = memo(
  (props: { runnerGroup: RunnerGroup; currentIndex: number; nextIndex: number; panelColorStyle: ColorStyle }) => {
    const setNextRunnerGroupIndex = useCallback((currentIndex: number, nextIndex: number, arrayIndex: number) => {
      if (currentIndex !== arrayIndex && nextIndex !== arrayIndex) {
        nodecg.sendMessage('setNextRunnerGroupIndex', arrayIndex)
      }
    }, [])
    const id = String(props.runnerGroup.group).padStart(2, '0')
    const title = props.runnerGroup.title.join('')
    const runnerNames = props.runnerGroup.runners.map((data, i) => <p key={`runnerNames${i.toString}`}>{data.name}</p>)
    ...
    return (
      <StSelectablePanel
        ...
        onClick={setNextRunnerGroupIndex.bind(this, props.currentIndex, props.nextIndex, props.runnerGroup.group)}>
        <p>
          {id} : {title}
        </p>
        <StNameContainer>
